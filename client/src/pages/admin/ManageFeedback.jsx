...
export default function ManageFeedback() {
  ...
  function submitResponse(id) {
    ...
    const updated = feedback.map((f) => f.id === id ? { ...f, response, status: "Resolved" } : f);
    saveFeedback(updated);
    setFeedback(updated);
    setSelected(null);
    setResponse("");
  ...
  const filtered = feedback.filter((f) => {
    ...
    const matchSearch = f.userName.toLowerCase().includes(search.toLowerCase()) || f.message.toLowerCase().includes(search.toLowerCase());
    return matchFilter && matchSearch;
  });

  const counts = STATUSES.reduce((acc, s) => ({ ...acc, [s]: feedback.filter((f) => f.status === s).length }), {});
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-5">
          <input type="text" placeholder="Search by name or message…" value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          <div className="flex gap-2 flex-wrap">
            {categories.map((c) => (
              <button key={c} onClick={() => setFilter(c)}
                className={`px-3 py-2 rounded-xl text-xs font-semibold border transition ${filter === c ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-200 hover:border-slate-400"}`}>
                {c}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          {filtered.length === 0 && <p className="text-center py-10 text-slate-400 text-sm">No feedback found.</p>}
          {filtered.map((f) => (
            <div key={f.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                {/* Category */}
                <span className="inline-flex items-center rounded-full bg-slate-100 border border-slate-200 px-2.5 py-0.5 text-xs font-semibold text-slate-600">{f.category}</span>
                ...
                {/* Expand button */}
                <button onClick={() => openRow(f.id)} className="text-xs font-semibold text-blue-600 hover:underline ml-auto sm:ml-0 flex-shrink-0">
                ...
                <div className="border-t border-slate-100 bg-slate-50 px-5 py-4 space-y-3">
                  ...
                  <div>
                    ...
                    <textarea value={response} onChange={(e) => setResponse(e.target.value)} rows={3}
                      placeholder="Type a response to send to the community member…"
                  ...
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => { setSelected(null); setResponse(""); }} className="px-4 py-2 text-xs font-semibold text-slate-600 border border-slate-200 rounded-xl hover:bg-white transition">Cancel</button>
                    <button onClick={() => submitResponse(f.id)} className="px-4 py-2 text-xs font-semibold bg-slate-900 text-white rounded-xl hover:bg-slate-700 transition">Save Response</button>
                  </div>
                </div>
              )}
            </div>
